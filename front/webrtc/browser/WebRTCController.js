...
// 여기다 다 박고, ref를 아래로 보내주는게 최고인듯 하다. 
  ...
  const startSignaling = async (signalRoomName) => {
    displaySignalMessage("starting signaling...  signalroom name :" + signalRoomName);


    if(!owner){
      console.log('new peerconnection fired on viewer side');
      socket.emit("newPeerConnection", {chatRoom: "a" + queryId,
      signal_room:signalRoomName})
    }
...
//            , mediaConstraints); 			
    ...
    peerConnection.onicecandidate = async (evt) => {
      ...
      if (evt.candidate)
        socket.emit(owner ? "broadcaster_icecandidate" : "viewer_icecandidate", {
          type: "ice candidate",
  ...
  socket.on("connectWebRTC", ({signal_room})=>{
    ...
    if(owner){
      ...
      socket.on("viewer_signaling_message", async (data) => {
        displaySignalMessage("Signal received: from viewer_signaling_message : " + data);
        ...
        if (data.type && data.type != "user_here") {
          ...
          if (message.sdp) {
            ...
            await peerConnection.setRemoteDescription(
              ...
              console.log('peerConnection.remoteDescription.type :', peerConnection.remoteDescription)
    ...
    if(!owner){
      socket.on("broadcaster_icecandidate", async (data)=>{
        try{
          ...
          await peerConnection.addIceCandidate(await new RTCIceCandidate(message.candidate));
        }catch(e){
